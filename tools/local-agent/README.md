...
### 가장 쉬운 방법 (Windows · 터미널 불필요)
1. **최초 1회** — `tools/local-agent` 폴더의 **`SETUP.bat` 더블클릭**.
   Node.js·Claude 설치를 자동 처리하고 로그인 화면까지 열어줍니다(로그인 클릭만 본인이 1회).
2. **그 이후 매번** — **`START.bat` 더블클릭** → 서버가 켜지고 브라우저가 자동으로 열립니다.
   종료는 같이 뜬 검은 창을 닫으면 됩니다.
